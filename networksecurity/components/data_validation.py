from networksecurity.Entity.artifact_entity import DataIngestoinArtifacts,DataValidationArtifacts
...
class DataValidation:
    def __init__(self,data_ingestion_artifact:DataIngestoinArtifacts,
    ...
    def initiate_data_validation(self)->DataIngestoinArtifacts:
        try:
            ...
            if not status:
                error_message=f"Train data frame does nnot contain all columns"
            ...
            if not status:
                error_message=f"Train data frame does not contain numerical columns"

            status=self.numerical_column_exist(dataframe=test_dataframe)
            if not status:
                error_message=f'Test data frame does not contain numerical columns'
            ...
            data_validation_aritfacts=DataValidationArtifacts(
            ...
            return data_validation_aritfacts
